/**
 * Reach Validator Utility
 *
 * Validates Watershed Reach Format (WRF) JSON files against the schema
...
// Compile schema
const validateSchema = ajv.compile(reachSchema);
...
const TRANSITION_TYPES = ['waterfall', 'slotCanyon', 'splash'];

// Shader category mapping
const SHADER_CATEGORIES = ['generative', 'reactive', 'transition', 'filter'];

// Flow map format mapping
const FLOW_MAP_FORMATS = ['png', 'raw', 'json'];

/**
 * Main validation function
 */
export function validateReach(reachData: any): ValidationResult {
  const errors: ValidationError[] = [];
  const warnings: ValidationError[] = [];

  // 1. Schema validation
  const schemaValid = validateSchema(reachData);
  ...
  if (schemaValid) {
    validateSemantics(reachData, errors, warnings);
  }
 ...
 */
function validateSemantics(reachData: any, errors: ValidationError[], warnings: ValidationError[]): void {
  const { reachId, metadata, world, segments, spawns, requiredAssets, transition } = reachData;
  ...
  // Validate transition segment index exists
  if (transition?.segmentIndex !== undefined && segments) {
    const hasTransitionSegment = segments.some((seg: any) => seg.index === transition.segmentIndex);
    if (!hasTransitionSegment) {
      errors.push({
        field: 'transition.segmentIndex',
        error: `Transition references invalid segment index ${transition.segmentIndex}`,
        suggestion: `Use a segment index between 0 and ${(world?.track?.totalSegments || 1) - 1}`,
      });
    }

    // Warn if transition is not the last segment
    const lastIndex = (world?.track?.totalSegments || segments?.length || 1) - 1;
    if (transition.segmentIndex !== lastIndex) {
      warnings.push({
        field: 'transition.segmentIndex',
        error: `Transition segment (${transition.segmentIndex}) is not the final segment (${lastIndex})`,
        suggestion: 'For waterfall bottleneck transitions, the transition should typically be the last segment',
      });
    }
  }
  ...
  // Validate requiredAssets have unique IDs
  if (requiredAssets) {
    const validateUniqueIds = (items: any[] | undefined, category: string) => {
      if (!items) return;
      const ids = new Set<string>();
      for (let i = 0; i < items.length; i++) {
        if (ids.has(items[i].id)) {
          errors.push({
            field: `requiredAssets.${category}[${i}].id`,
            error: `Duplicate ${category} id: ${items[i].id}`,
            suggestion: 'Ensure each asset has a unique id within its category',
          });
        }
        ids.add(items[i].id);
      }
    ...
    validateUniqueIds(requiredAssets.textures, 'textures');
    validateUniqueIds(requiredAssets.models, 'models');
    validateUniqueIds(requiredAssets.audio, 'audio');
    validateUniqueIds(requiredAssets.shaders, 'shaders');
    validateUniqueIds(requiredAssets.flowMaps, 'flowMaps');
  }

  // Warnings for unusual values
  if (metadata?.estimatedDuration > 1200) {
    warnings.push({
      field: 'metadata.estimatedDuration',
      error: 'Very long estimated duration (>20 minutes)',
      suggestion: 'Consider splitting into multiple Reaches',
    });
  }

  if (world?.track?.totalSegments > 30) {
    warnings.push({
      field: 'world.track.totalSegments',
      error: 'Large number of segments may impact streaming performance',
...
function getSuggestionForError(error: any): string | undefined {
  ...
  if (message.includes('must be equal to constant')) {
    ...
    if (path.includes('category')) {
      return `Valid shader categories: ${SHADER_CATEGORIES.join(', ')}`;
    }
    if (path.includes('format')) {
      return `Valid flow map formats: ${FLOW_MAP_FORMATS.join(', ')}`;
    }
  }
