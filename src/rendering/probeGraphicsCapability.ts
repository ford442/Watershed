...
export type GraphicsCapabilityReason =
  ...
  /** Neither request produced a context. */
  | 'no-context';
...
/**
 * Attributes the probe asks for.
 *
 * Deliberately minimal — this request exists to answer "will this machine give
 * us a context at all, and does it pass the caveat check", not to match the
 * game's real attribute set. Asking for less makes a failure mean what we think
 * it means.
 */
...
export function probeGraphicsCapability(input: GraphicsProbeInput = {}): GraphicsCapability {
  ...
  const strict = input.skipStrictAttempt
    ? null
    : (() => {
        const canvas = createCanvas();
        if (!canvas) return { ok: false, statusMessage: null } satisfies ProbeAttemptResult;
        return attemptContext(canvas, {
          ...PROBE_ATTRIBUTES,
          powerPreference: HARDWARE_ENVELOPE.powerPreference,
          failIfMajorPerformanceCaveat: true,
        });
      })();
  ...
  const relaxedCanvas = createCanvas();
  const relaxed = relaxedCanvas
    ? attemptContext(relaxedCanvas, {
        ...PROBE_ATTRIBUTES,
        powerPreference: DEGRADED_ENVELOPE.powerPreference,
        failIfMajorPerformanceCaveat: false,
      })
    : ({ ok: false, statusMessage: null } satisfies ProbeAttemptResult);

  const statusMessage = relaxed.statusMessage ?? strict?.statusMessage ?? null;
