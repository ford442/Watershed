...
describe('probeGraphicsCapability', () => {
  it('reports the hardware tier when the strict request succeeds', () => {
    ...
    // And it asks exactly what THREE.WebGLRenderer will ask, attribute for
    // attribute. A probe that requests less can pass where the renderer fails.
    expect(log.attempts[0]).toEqual(rendererContextAttributesFor(HARDWARE_ENVELOPE));
  });
  ...
  it('falls to the degraded tier when only the relaxed request succeeds', () => {
    ...
    expect(log.attempts[1]).toEqual(rendererContextAttributesFor(DEGRADED_ENVELOPE));
  });
  ...
  it('reports the unsupported tier and the browser status message when both fail', () => {
    ...
    // strict, relaxed, and one bare request to classify the failure.
    expect(log.attempts).toHaveLength(3);
    expect(log.attempts[2]).toEqual({});
  ...
  it('separates "no WebGL2 here" from "not with those attributes"', () => {
    // A bare context succeeds, but nothing with depth + stencil does. Same
    // outcome — the game cannot mount — but a very different sentence on the
    // unsupported screen, because this browser looks healthy to the user.
    const { factory, log } = createCanvasFactory((attributes) =>
      Object.keys(attributes).length === 0
        ? { ok: true }
        : { ok: false, statusMessage: 'Requested attributes are not supported' }
    );
    const capability = probeGraphicsCapability({ createCanvas: factory });

    expect(capability.tier).toBe('unsupported');
    expect(capability.reason).toBe('attributes-rejected');
    expect(capability.statusMessage).toBe('Requested attributes are not supported');
    // The bare probe context is released like every other.
    expect(log.released).toBe(1);
  });
