...
export interface WaterfallParticlesProps {
    ...
    particleDensity?: number; // 0.0-1.0 from segment config
    fanAngle?: number; // degrees of horizontal spread; 0 = straight down
}
...
export default function WaterfallParticles({
    ...
    useFrame((state, delta) => {
        ...
        if (meshRef.current.material) {
            (meshRef.current.material as THREE.MeshStandardMaterial).opacity = 0.6 * fadeAlphaRef.current;
        }
    ...
    return (
        <group position={[0, -height / 2, 0]}>
            <instancedMesh
                ref={meshRef}
                args={[undefined, undefined, 500]} // Max 500 particles
                frustumCulled={false}
            >
