...
export interface FloatingDebrisProps {
  path?: THREE.CatmullRomCurve3 | null; // The CatmullRomCurve3
  waterLevel?: number;
  count?: number;
  seed?: number;
}

type DebrisType = 'driftwood' | 'pinecone';

interface DebrisItem {
  key: number;
  /** Debris kind — named distinctly from Rapier's per-instance `type` (RigidBodyTypeString)
   * override, which `InstancedRigidBodies` reads directly off each spread instance object. */
  ...
  seed = 0
    ...
    const debris = useMemo<DebrisItem[]>(() => {
        ...
        for (let i = 0; i < count; i++) {
            ...
            const debrisType: DebrisType = seededRandom(currentSeed++) > 0.7 ? 'driftwood' : 'pinecone';
    ...
    const driftwoodInstances = useMemo(() => debris.filter(d => d.debrisType === 'driftwood'), [debris]);
    const pineconeInstances = useMemo(() => debris.filter(d => d.debrisType === 'pinecone'), [debris]);
    ...
    useFrame((state, delta) => {
        ...
        const updateInstances = (instances: DebrisItem[], ref: React.RefObject<(RapierRigidBody | null)[]>) => {
