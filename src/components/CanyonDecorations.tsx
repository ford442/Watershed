...
export default function CanyonDecorations({
    ...
    const smallBouldersRef = useRef<THREE.InstancedMesh>(null);
    const wallRocksRef = useRef<THREE.InstancedMesh>(null);
    ...
    useEffect(() => {
        ...
        if (smallBouldersMesh && decorationData.smallBoulders.length > 0) {
            decorationData.smallBoulders.forEach((boulder, i) => {
                ...
                smallBouldersMesh.setMatrixAt(i, matrix);
            });
            smallBouldersMesh.instanceMatrix.needsUpdate = true;
        }
        ...
        if (wallRocksMesh && decorationData.wallRocks.length > 0) {
            decorationData.wallRocks.forEach((rock, i) => {
                ...
                matrix.compose(position, quaternion, scale);
                wallRocksMesh.setMatrixAt(i, matrix);
            });
            wallRocksMesh.instanceMatrix.needsUpdate = true;
        }
        ...
        if (vegetationMesh && decorationData.vegetation.length > 0) {
            decorationData.vegetation.forEach((veg, i) => {
                ...
                matrix.compose(position, quaternion, scale);
                vegetationMesh.setMatrixAt(i, matrix);
            });
        ...
        if (hangingGrowthMesh && decorationData.hangingGrowth.length > 0) {
            decorationData.hangingGrowth.forEach((growth, i) => {
                ...
                matrix.compose(position, quaternion, scale);
                hangingGrowthMesh.setMatrixAt(i, matrix);
                hangingGrowthMesh.setColorAt(i, new THREE.Color(growth.color));
            });
            hangingGrowthMesh.instanceMatrix.needsUpdate = true;
            if (hangingGrowthMesh.instanceColor) {
                hangingGrowthMesh.instanceColor.needsUpdate = true;
            }
        }
    }, [decorationData]);
