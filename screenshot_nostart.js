...
async function run() {
  ...
  // Hide the start menu overlay without clicking start
  await page.addStyleTag({ content: '.start-menu-overlay { display: none !important; }' });
  ...
  if (dataUrl) {
    fs.writeFileSync('/content/watershed/canvas_nostart.png', Buffer.from(dataUrl.split(',')[1], 'base64'));
    console.log('Saved canvas_nostart.png');
  }
  await page.screenshot({ path: '/content/watershed/screenshot_nostart.png', fullPage: false });
  console.log('Saved screenshot_nostart.png');
  await browser.close();
}
