...
**Resolved:** `public/rapier.wasm` has been deleted and its line dropped from
`EXPECTED_PASSENGERS` — −1.57 MB per deploy. `deploy.py` had no reference to it, so nothing else
needed to change.
...
ordering. **Resolved: `public/index.html` has been deleted**, and the guard's `PASSENGER_EXCLUDED`
set no longer carries it — if it is ever resurrected, the passenger-inventory check now fails
loudly on it instead of silently excluding it.
...
**Resolved (2026-09).** All 23 were replaced in place by project-synthesized foley
(`scripts/generate-sounds.mjs`, byte-stable; recipes and license in `public/sounds/README.md`):
**23 distinct sha256 values**, 681,264 bytes. Filenames are unchanged, so `SOUND_DEFS` URLs did
not churn. `src/systems/audio/soundDefs.test.ts` now fails if two file entries share a hash —
names that legitimately share a payload say so with `aliasOf`. The replacement exercised exactly
the class above: `ambient_canyon.mp3` and `ambient_wind.mp3` are both 128,731 bytes, so a
size-only skip would have been wrong. `deploy.py` size-skips only `assets/<name>-<hash>.<ext>`,
so every `sounds/` file still uploads on every deploy. `public/sounds/README.md` is a new pinned
passenger in `scripts/check-build-manifest.mjs`.
...
| `build/rapier.wasm` | **Deleted.** | Was guarded as a passenger for contract completeness, but it was 1.57 MB of dead, divergent weight ([§6.1](#61-rapiers-wasm-ships-three-times)). Removed, along with its `EXPECTED_PASSENGERS` line. |
| `public/index.html` | **Deleted.** | Was not a passenger — an accident that Vite's copy ordering used to hide ([§6.2](#62-publicindexhtml-is-a-blank-page-waiting-to-happen)). |
