...
When path generation fails or returns invalid values, geometry creation in `TrackSegment/` can produce NaN values in buffer attributes, causing the "Computed radius is NaN" error.
