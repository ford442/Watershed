...
- **`three` and `three/webgpu` share one core since r171** — both entries import `build/three.core.js`, so at the r178 pin `(await import('three/webgpu')).DirectionalLight === THREE.DirectionalLight`, and r168's `WARNING: Multiple instances of Three.js being imported` is gone from the console. Through r170 `three/webgpu` carried its own copy of the core (`resolve.dedupe` could not merge two entry files), and `NodeLibrary` indexed both its tables by something that did not survive that split. [`src/rendering/nodeLibraryBridge.ts`](../../src/rendering/nodeLibraryBridge.ts) re-registers `three`'s side after `renderer.init()`. **At r178 it bridges nothing** — `nodeLibraryBridge.test.ts` pins that against the real `three.webgpu.js` bundle, not the test double — and it stays as the guard for the next bump:
  - **Lights** — `lightNodes` is a WeakMap keyed on the light class, so with two cores every light R3F built from `three` missed. r168's `LightsNode` then reached `new null( light )` and threw on the first lit material; `?material=tsl` rendered an empty canvas because of it. (r178 warns and skips the light instead.)
  - **Materials** — r168 keyed `materialNodes` by `materialClass.name` while lookups use `material.type`. Those agree only until a minifier renames the class, so **production only**, a plain material logged `NodeMaterial: Material "…" is not compatible.` and was silently replaced by a blank `NodeMaterial`. r178's `StandardNodeLibrary` keys by the type strings itself, which is what the bridge used to add.
  - The library lives at `renderer.library` since r178 (r168: `renderer.nodes.library`), and `addMaterial()` takes the type string. Any future class-identity lookup needs the same treatment; `.isXxx` flag checks are unaffected.
- **A zero-count `InstancedMesh` will not compile under a node material.** `InstanceNode.setup` builds the instance matrices as a UBO whenever `count <= 1000`, and the GLSL backend prints the array size as `bufferCount > 0 ? bufferCount : ''` — so `count === 0` emits `uniform NodeBuffer_N { mat4 bufferN[]; };`, an unsized array in an interface block, which GLSL ES 3.00 rejects. Every empty decoration layer killed its own program (~103 link failures on a boot). Verified in isolation on r168: count 0 fails, counts 5 and 1200 are clean; the classic `WebGLRenderer` never cared. r178's `InstanceNode` sizes that buffer as `Math.max( count, 1 )`, which removes the unsized array on paper — but the UBO still keeps whatever size the program was compiled at, and the render-object cache key only changes when `count` crosses 1, so the guards stay until a runtime re-measure says otherwise. Two guards cover it — **mount instanced geometry through them, never `<instancedMesh>` or drei's `<Instances>` directly**:
