...
# Artifact stamp — sha256 over BOTH halves of the pair (glue JS + wasm binary, plus
# the pthread shim when present). Emscripten glue and .wasm are only valid together;
# the stamp must move if EITHER half changes, so a split-provenance deploy (#402) is
# detectable. Written for every configuration, threads included — a --threads build
# used to leave the previous single-threaded stamp in place, which is exactly the kind
# of silent drift this stamp exists to prevent.
if [ -f "$PUBLIC_DIR/watershed_native.wasm" ]; then
    STAMP_INPUTS=("$OUTPUT_JS" "$PUBLIC_DIR/watershed_native.wasm")
    if [ -f "$PUBLIC_DIR/watershed_native.worker.js" ]; then
        STAMP_INPUTS+=("$PUBLIC_DIR/watershed_native.worker.js")
    fi
    STAMP=$(cat "${STAMP_INPUTS[@]}" | sha256sum | cut -c1-16)
