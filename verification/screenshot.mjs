/**
 * screenshot.mjs — Quick single-frame WebGL screenshot + console log capture.
 *
 * Usage:
 *   pnpm dev
 *   node verification/screenshot.mjs [url] [output.png]
 *
 * Output: verification/output/screenshot.png (+ .log.txt)
 */
...
const BASE_URL = process.argv[2] || process.env.WATERSHED_URL || 'http://localhost:3000';
const URL = BASE_URL.includes('?')
  ? `${BASE_URL}&renderer=webgl&no-pointer-lock=1`
  : `${BASE_URL}?renderer=webgl&no-pointer-lock=1`;
const OUT = process.argv[3] || path.join(OUT_DIR, 'screenshot.png');
